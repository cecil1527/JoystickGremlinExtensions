...
class StepperVals:
    ...
    def FromSpecificVals(specific_vals, val_range, axis_range=(-1.0, 1.0)):
        ...
        vals = [
            utils.lerp(val_range[0], axis_range[0], val_range[1], axis_range[1], v)
            for v in specific_vals
        ]
        return vals

    @staticmethod
    def FromRange(
        num_vals: int, val_range, axis_range=(-1.0, 1.0), curvature: float = 0
    ):
        """
...
class StepperAxis:
    def __init__(
        self, axis_id: int, axis_vals, initial_idx: int = None, device_id: int = 1
    ) -> None:
        """
    ...
    def toggle_default_idx(
        self, default_idx: int, other_idx: int, epsilon: float = 1e-3
    ):
        """
